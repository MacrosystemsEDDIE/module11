    ...
    track.fit_arima2$prev = track.fit_arima2$curr
      ...
      output$plot_models <- renderPlotly({ 
        ...
        p <- ggplot() +
          annotate("text", x = 10,  y = 10,
                   size = 6,
                   label = "Looks like you've chosen new regressors!\nPlease click 'Fit ARIMA' in Obj. 7 and then\n 'Fit additional models' to regenerate this plot!") + 
          theme_void()+
          theme(panel.grid = element_blank(),
                axis.line = element_blank())
  ...
  observe({
    ...
    output$plot_models <- renderPlotly({ 
      ...
      p <- ggplot() +
        annotate("text", x = 10,  y = 10,
                 size = 6,
                 label = "Please click 'Fit additional models' to regenerate this plot \nwith your new regressors!") + 
        theme_void()+
        theme(panel.grid = element_blank(),
              axis.line = element_blank())
      ...
      output$plot_models <- renderPlotly({ 
        ...
        p <- ggplot() +
          annotate("text", x = 10,  y = 10,
                   size = 6,
                   label = "Please click 'Fit ARIMA' in Obj. 7 and then\n 'Fit additional models' to regenerate this plot!") + 
          theme_void()+
          theme(panel.grid = element_blank(),
                axis.line = element_blank())
  ...
  observe({
    ...
    output$nnetar_order <- renderText({
      ...
      output$nnetar_order <- renderText({
        ...
        return("Please click 'Fit ARIMA' in Obj. 7 and then 'Fit additional models'.")
