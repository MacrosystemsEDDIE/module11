...
pred_rw <- forecast(rw, new_data = new_data) %>%
  hilo() %>%
...
ARIMA_pred_plot <- ggplot()+
  xlab("")+
  ylab("Chla (mg/L)")+
  geom_point(data = new_data, aes(x = datetime, y = chla, fill = "obs"))+
...
new.data <- wide_dat %>%
        filter(!datetime %in% model_df4$datetime) %>% # using a 70:30 split here
        mutate(across(amm:flow, list(zscore = ~as.numeric(scale(.)))))
pred_rw0 <- forecast(my.rw, new_data = new.data) %>%
        hilo() 
      pred_rw <- pred_rw0 %>%
        mutate(lower = `95%`$lower,
               upper = `95%`$upper) %>%
        select(.model, datetime, .mean, lower, upper)
      pred_r <- pred_rw0 %>% pull(chla)
      check <- distributional::parameters(pred_r)
